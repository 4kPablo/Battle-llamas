  ...
  if (width > 640) {
    ...
    return (<>
      <div className={"flex gap-1 md:gap-10 items-center " + classNames.join(" ")}>
        <img className="flex flex-[2] w-full min-w-[250px] h-full min-h-[250px] rounded-xl md:rounded-3xl" src={img} />
        <div className="flex flex-[5] flex-col justify-center p-4">
          <h2 className=' text-left text-2xl lg:text-4xl font-bold text-white mb-3'>{title}</h2>
          <p className=' text-left text-lg lg:text-2xl text-[#D9BD8B]'>{description}</p>
        </div>
      </div>
   </> );
